...
It's easier to process FPV data into KITTI format and then let this model handle it like normal.
It stacks images in some weird way and it's not as easy as changing Dataset to our own.

To import data, you need to use `import_fpv.py` script. Put there which datasets you want to import.
The script copies only left images from stereocamera, moves into correct folder and names them correctly.
It also takes groundtruth data and 1) represents it in rotation matrix form instead of (translation, quaternion) 2) expresses everything relative to first frame.
It also automatically disregards images with no corresponding groundtruth label.

After importing you can continue with the next steps, preprocessing, etc.
